...
class AttentionGANModel(BaseModel):
    ...
    def backward_D_basic(self, netD, real, fake, rotate_real, rotate_fake):
        ...
        # Fake
        ####### Self-Supervised Task #######   
        _, pred_fake, rotation_fake, _ = netD(rotate_fake.detach())
        #_, pred_fake, rotation_fake, _ = netD(fake.detach(), rotate_fake.detach())
        ####### Self-Supervised Task #######   
        ...
        loss_D = self.add_rotation_loss_d(self.d_class_loss, loss_D)
        ####### Self-Supervised Task #######
        loss_D.backward()
        return loss_D
    ...
    def backward_D_A(self):
        ...
        self.loss_D_A = self.backward_D_basic(self.netD_A, self.real_B, fake_B,  self.rotate_real_B, self.rotate_fake_B)

    def backward_D_B(self):
        """Calculate GAN loss for discriminator D_B"""
        fake_A = self.fake_A_pool.query(self.fake_A)
        self.loss_D_B = self.backward_D_basic(self.netD_B, self.real_A, fake_A,  self.rotate_real_A, self.rotate_fake_A)
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            #_, _, _, _, _, _, _, _, _  = self.netG_A(self.real_B, self.rotate_real_B)
            self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            self.idt_B, _, _, _, _, _, _, _, _, _, _, \
            _, _, _, _, _, _, _, _, _, _, \
            _, _, _, _, _, _, _, _, _  = self.netG_B(self.real_A)
            #_, _, _, _, _, _, _, _, _  = self.netG_B(self.real_A, self.rotate_real_A)
        ...
        #_, pred_fake_A, rotation_fake_A, _ = self.netD_A(self.real_B, self.rotate_fake_B)
        self.loss_G_A = self.criterionGAN(pred_fake_A, True)
        # GAN loss D_B(G_B(B))
        _, pred_fake_B, rotation_fake_B, _ = self.netD_B(self.rotate_fake_A)
        #_, pred_fake_B, rotation_fake_B, _ = self.netD_B(self.real_A, self.rotate_fake_A)
        ...
        self.loss_G = self.add_rotation_loss_g(self.g_class_loss_A, self.loss_G)
        self.g_class_loss_B = self.auxiliary_ls_rotation_loss(self.fake_A.size(0), rotation_fake_B)
        self.loss_G = self.add_rotation_loss_g(self.g_class_loss_B, self.loss_G)
        ####### Self-Supervised Task #######
    ...
    def transfer_rotation_loss(self):
        d_class_loss = self.d_class_loss
        g_class_loss_A = self.g_class_loss_A
        g_class_loss_B = self.g_class_loss_B
        return d_class_loss, g_class_loss_A, g_class_loss_B
    ####### Self-Supervised Task #######   
