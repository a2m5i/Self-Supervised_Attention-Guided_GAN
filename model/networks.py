...
class ResnetGenerator_our(nn.Module):
    ...
    def forward(self, input):
        ...
        attention1_ = attention[:, 0:1, :, :]   # [A, 0, B, C]
        attention2_ = attention[:, 1:2, :, :]   # [A, 1, B, C]
        attention3_ = attention[:, 2:3, :, :]   # [A, 2, B, C]
        attention4_ = attention[:, 3:4, :, :]   # [A, 3, B, C]
        attention5_ = attention[:, 4:5, :, :]   # [A, 4, B, C]
        attention6_ = attention[:, 5:6, :, :]   # [A, 5, B, C]
        attention7_ = attention[:, 6:7, :, :]   # [A, 6, B, C]
        attention8_ = attention[:, 7:8, :, :]   # [A, 7, B, C]
        attention9_ = attention[:, 8:9, :, :]   # [A, 8, B, C]
        attention10_ = attention[:, 9:10, :, :] # [A, 9, B, C]n

        attention1 = attention1_.repeat(1, 3, 1, 1)
        # print(attention*.size()) # [3, 3, 256, 256]
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        ####### Self-Supervised Task #######
        self.discrimination_ = nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)  # output 1 channel prediction map
        self.rotation_ = nn.Conv2d(ndf * nf_mult, 4, kernel_size=kw, stride=1, padding=padw)  # output 4 channel prediction map for 4 rotations
        ####### Self-Supervised Task #######

    def forward(self, input):
        """Standard forward."""
        ####### Self-Supervised Task #######
        batch_len = int(input.size(0)/4)
        out_features = self.model(input)
        discrimination_output = self.discrimination_(out_features[:batch_len, :, :, :])
        rotation_output = self.rotation_(out_features)
        return self.sigmoid(discrimination_output), discrimination_output, rotation_output, self.softmax(rotation_output) 
        ####### Self-Supervised Task #######
    
    #def forward(self, input, rotate_input):
        #"""Standard forward."""
        ####### Self-Supervised Task #######
        #discrimination_output = self.discrimination_(input)
        #rotation_output = self.rotation_(rotate_input)
        #return self.sigmoid(discrimination_output), discrimination_output, rotation_output, self.softmax(rotation_output) 
        ####### Self-Supervised Task #######
